...
# li = tuple(random.randrange(1, 1000, 1) for i in range(200))
...
def execute_parallel(source, param, main_fun):
    ...
    for i in range(0, len(param['urls']), source.parallel):
        if sleep_time > 0 and source.max_calls_per_reset_time - source.parallel < counter:
            print(f'musimy poczekac {sleep_time:.5f} sec bo exec time is {exec_time: .5f}')
            time.sleep(sleep_time)
            counter = 0
            st = time.time()

        pool_list = []  # param['urls']
        headers = param['headers']
        params = param['params']
        j = 0
        for j in range(0, min(source.parallel, len(param['urls']) - j - i), 1):
            if len(param['urls']) < i + j + 1:
                pass
                # break
...
def airly_request(li):
    # reset_time = 20
    # max_calls_per_reset_time = 100
    # max_cals_per_day = 1000
    # credentials = ''  # _get_credentials()
    # parallel = 4  # how may api calls in parallel

    source = Airly
    param = {'urls': li['urls'], 'params': li['params'], 'headers': li['headers']}
    ...
    # return {'installationId':Airly , 'all_sensors':all_sensors}
    # print(all_sensors)
    return all_sensors


aa = AirlyRequest()
installations = ['984', '655', '97', '1088', '1035', '736', '987', '706', '992', '835', '346', '799', '90', '802',
...
li = aa.construct_dict_of_url_installation(installations)  # from airly installations
# print(di)
# slipper(10)

#

ee = Airly.get_credentials()

# del(ee['airly_key'])
# with pg.connect(**ee) as connection:
#     cursor = connection.cursor()
#


results = airly_request(li)
# list_of_json_results = [{'measure': json.dumps(one_measure)} for installation in results for one_measure in
#                         installation]
...
list_of_json_results = [[one_measure['fromdatetime'], one_measure['tilldatetime'], json.dumps(one_measure)] for
                        ...
                        installation]
# list_of_json_results = [one_measure for installation in results for one_measure in installation]
#
AirlyDatabase.import_measure_list_to_table(list_of_json_results)
# # TODO: insert data into postgres table with column defined as jsonb
